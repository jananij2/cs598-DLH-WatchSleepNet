"""Stage 2 preprocessing for the Multi-Ethnic Study of Atherosclerosis (MESA).

Reads the compressed per-subject NPZ files produced by ``extract_mesa.py``
(Stage 1), runs PPG peak detection via neurokit2, computes a per-sample IBI
time series, downsamples to 25 Hz, and writes the final ``.npz`` cache files
...
Output NPZ keys (same format as preprocess_mesa.py)
----------------------------------------------------
...
Usage::
    ...
    python process_mesa.py \\
        --signal_dir /remote/MESA_extracted \\
...
__all__ = ["process_mesa"]

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Constants (match preprocess_mesa.py)
# ---------------------------------------------------------------------------
...
def _process_subject(
    ...
    """Run PPG peak detection, IBI computation, and downsampling for one subject.

    Args:
        signal: Raw Pleth PPG array at ``fs`` Hz.
    ...
    if fs % _TARGET_FS == 0:
        ...
        # e.g. 256 Hz → 25 Hz requires polyphase resampling
...
def process_mesa(
    ...
    """Run IBI computation on extracted MESA NPZs and write final cache files.

    Args:
        signal_dir: Directory of NPZ files produced by ``extract_mesa.py``.
    ...
    signal_files = sorted(signal_dir.glob("mesa-*.npz"))
    if not signal_files:
        raise FileNotFoundError(f"No mesa-*.npz files found in {signal_dir}")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage 2: compute MESA IBI from extracted NPZs and write final cache files."
    )
    parser.add_argument("--signal_dir", required=True,
                        help="Directory of NPZ files from extract_mesa.py.")

"""Offline preprocessing script for the Sleep Heart Health Study (SHHS).

Processes both SHHS1 and SHHS2 visits end-to-end: reads each subject's EDF
file, extracts the ECG channel, detects R-peaks via biosppy, computes a
per-sample IBI time series, downsamples to 25 Hz, and writes a per-subject
``.npz`` cache file compatible with the WatchSleepNet ``SSDataset`` loader.
...
Output is written to the same directory as MESA IBI output so both datasets
...
# Subject excluded in the original WatchSleepNet data_setup.py
...
def _read_edf_ecg(
    ...
    """Read ECG signal and sleep stage labels from one SHHS EDF + XML pair.

    Args:
        edf_path: Path to the ``*-PSG.edf`` file (or SHHS EDF equivalent).
    ...
    Returns:
        ...
        - ``signal``: float64 ndarray of shape ``(n_samples,)`` containing
          the raw ECG at its native sampling rate (typically 125 Hz).
...
def _compute_ibi_ecg(signal: np.ndarray, fs: float) -> np.ndarray:
...
def read_shhs_subject(
    edf_path: Path, ann_path: Path, ahi: float
) -> Tuple[np.ndarray, int, np.ndarray, float]:
    """End-to-end pipeline for a single SHHS subject.

    Reads ECG from EDF, extracts IBI, downsamples to 25 Hz, and returns
    ...
    """
    signal, fs, stages = _read_edf_ecg(edf_path, ann_path)
    ibi = _compute_ibi_ecg(signal, fs)
...
def _process_one(args: Tuple) -> Tuple[str, str]:
    ...
    try:
        ibi_ds, fs_out, stages_ds, ahi_val = read_shhs_subject(
...
def preprocess_shhs(
    ...
    """Preprocess all SHHS subjects and write per-subject ``.npz`` cache files.

    Processes both SHHS1 and SHHS2 visits. Already-existing output files are
    skipped (idempotent re-runs). One subject (``shhs1-204822``) is excluded
    to match the original WatchSleepNet data loader.
    ...
    # The harmonized CSV contains one row per visit (visitnumber 1 and 2).
    # AHI is only recorded in visit 1; filtering here prevents the dict(zip)
    # from overwriting valid visit-1 AHI values with NaN from visit-2 rows.
    ...
    # Collect all (subject_key, edf_path, ann_path, ahi) tuples
    ...
    for visit in _VISITS:
        ...
        for edf_path in sorted(edf_dir.glob("*.edf")):
            sid = edf_path.stem.split("-")[1]  # "shhs1-200001" → "200001"
    ...
    try:
        with Pool(n_workers) as pool:
            ...
            if tqdm:
                iterator = tqdm(iterator, total=len(work_items), desc="SHHS")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess SHHS dataset to per-subject IBI NPZ cache files."
    )
    parser.add_argument(
        "--src_dir",
        required=True,
